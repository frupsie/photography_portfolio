 ...
 *
 * Staged variant: same choreography as the live act, but the map's pins are
 * live — clickable, focusable, tooltipped — via GlobeSectionV2's
 * `interactivePins`. Live Act2Journey.jsx is untouched.
 *
 * Uses GlobeSectionV2 in "passive" mode (no chrome,
...
const GlobeSectionV2 = lazy(() => import('./GlobeSectionV2'));
...
export default function Act2JourneyV2() {
  ...
  useEffect(() => {
    ...
    const ctx = gsap.context(() => {
      ...
      // opacity. Near the end of the act, the counter dims TOGETHER with the
      // map (same target opacity) so the final stats stay readable rather than
      // disappearing — they ride the same dim-out as the rest of the scene.
      tl.to(mapWrapRef.current, { opacity: 1, ease: 'power2.out' }, 0)
        ...
        .to(counterRef.current, { opacity: 0.55, ease: 'power2.in' }, 0.90);
    }, sectionRef);
  ...
  return (
    <section ref={sectionRef} className="reel__act reel__act--2 reel__act--2v2">
